...
The current kernel contains exactly 235 project-authored Rust `unsafe` tokens,
...
| x86-64 APIC input/timer/network delivery and entry | 26 | Mapped LAPIC/I/O APIC registers and owned PIC/UART/PIT ports are accessed only from the pinned q35 profile; PIT channel 2 calibrates a masked local-APIC one-shot before the 50 ms lease is armed; the validated virtio INTx line is active-low/level-triggered; explicit IDT gates preserve interrupted integer and floating-point state; `sti; hlt; cli` closes the empty-check sleep race. |
| AArch64 GICv2 input/timer/network delivery and entry | 26 | The pinned `virt` GICv2 aperture is mapped RW/NX as device memory; distributor/CPU-interface operations are bounded by `GICD_TYPER`; PPI 30 carries the checked generic physical-timer deadline and one validated SPI carries virtio-net completion; the IRQ vector preserves all general and SIMD state; IRQ-masked `dsb; wfi` closes the pre-sleep handler race before pending dispatch returns to masked queue access. |
