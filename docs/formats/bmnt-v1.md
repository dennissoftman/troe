...
that source into `EFI/BOOT/VOLUMES.BMT`; the kernel reads and validates that
file before leaving UEFI boot services.
...
availability policy, activation mode, selector kind, and stable identifiers. A
whole-device selector is valid only for ext4 and carries its nonzero 128-bit
filesystem UUID. A GPT selector carries a nonzero GPT disk GUID, nonzero unique
partition GUID, and the filesystem identity. Ext4 identities are exact nonzero
128-bit UUIDs. FAT32 identities encode the nonzero 32-bit volume ID in
little-endian order followed by twelve zero bytes. Labels are never selectors.
...
`root` requires the ext4 profile and automatic activation; `boot` requires
FAT32. Other names describe
...
| 8 | 2 | name bytes | 1–32 |
| 10 | 1 | activation | 1 automatic, 2 manual |
| 11 | 5 | reserved | zero |
...
Read-write is encoded so provisioning policy is explicit, but parsing a
manifest does not grant write authority. The activator still requires the
selected provider, transport durability semantics, dirty-state policy, and
caller authority to permit it. Automatic matches attach while the namespace is
composed. Manual matches retain an already validated provider until the bounded
volume-control capability receives `mount NAME`; it cannot select a new device,
filesystem, or target path.
