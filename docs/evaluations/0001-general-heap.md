...
| [`embedded-alloc` 0.7 TLSF](https://docs.rs/embedded-alloc/0.7.0/embedded_alloc/struct.TlsfHeap.html) | Maintained Rust Embedded wrapper around `rlsf`, `no_std`, MIT/Apache-2.0. Its critical-section integration does not replace TROE's required UEFI fallback, ownership phase, counters, or machine lock, so the wrapper adds no useful boundary here. |
