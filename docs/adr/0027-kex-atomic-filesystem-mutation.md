...
atomic regular-file or symbolic-link removal, symbolic-link creation, and
same-provider regular-file hard-link creation. `begin-replace` resolves paths
...
`abort` consume the token plus bounded symbolic- and hard-link methods.
...
copies have explicit 64 KiB ceilings and are reclaimed before the shell resumes.

This is sufficient for current `write`, `rm`, and `ln` behavior and for small
runtime state files. Larger-runtime packaging, module loading, and durable
