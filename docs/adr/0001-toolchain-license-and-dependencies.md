...
tools. License kllm under Apache-2.0. Portable crates have no external runtime
dependencies. Use `uefi` 0.39.0 only at the firmware boundary and pin the graph
in `Cargo.lock`.
...
surface without differentiating kllm. A large host build framework was rejected
because standard Cargo and small deterministic tools cover the current needs.
