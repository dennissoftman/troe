...
use troe_fmt_gpt::{GptError, GptGuid, GptLimits, GptPartition, discover};
    ...
    match selector.kind() {
        ...
        SelectorKind::GptPartition => {
            let partition = {
                ...
                if selector
                    .disk_guid()
                    .map(troe_fmt_bmnt::StableIdentifier::bytes)
                ...
                let partition = gpt.partition_by_unique_guid(
                    troe_fmt_gpt::GptGuid::from_disk_bytes(partition_guid.bytes()),
                )?;
                (
                    partition.first_lba(),
...
mod tests {
    ...
    use troe_fmt_bmnt::{AccessMode, MountEntry, VolumeSelector, parse_manifest};
    use troe_fmt_gpt::GptLimits;
