...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    resolve_command.add_argument("--manifest", type=Path, action="append", required=True)
    resolve_command.add_argument("--output", type=Path)

    build = subparsers.add_parser("build", help="construct a canonical package artifact")
    ...
    explain = subparsers.add_parser("explain", help="explain declared authority and cost")
    explain.add_argument("--manifest", type=Path, required=True)

    plan_command = subparsers.add_parser("plan", help="derive a non-mutating activation plan")
    ...
    diagnostics = subparsers.add_parser("diagnostics", help="emit stable validation diagnostics")
...
def read_bounded(path: Path, maximum: int = 8 * 1024 * 1024) -> bytes:
    ...
    try:
        if path.is_symlink() or not path.is_file():
            raise ModelError("invalid-input", str(path), "must be a regular non-symlink file")
...
def execute(args: argparse.Namespace) -> dict[str, object]:
    ...
    if args.command in {"check", "diagnostics"}:
        ...
        return {
            "checked": [
                {"manifest_sha256": manifest.digest(), "name": manifest.name, "version": manifest.version.json()}
    ...
    if args.command == "build":
        ...
        payload = build_package(manifest, lock, read_bounded(args.artifact, 4 * 1024 * 1024))
        parse_package(payload)
        publish(args.output, payload)
        return {"bytes": len(payload), "package_sha256": sha256(payload), "target": lock.target}
    if args.command == "inspect":
        if args.manifest is not None:
            return manifest_summary(parse_manifest_file(args.manifest))
        manifest, lock, artifact = parse_package(read_bounded(args.package), str(args.package))
        return {
    ...
    if args.command == "plan":
        lock = parse_lock(read_bounded(args.lock), str(args.lock))
        manifests = load_manifests(args.manifest)
        catalog = {(manifest.name, manifest.version): manifest for manifest in manifests}
        if len(catalog) != len(manifests):
            raise ModelError("duplicate-package", "catalog", "duplicate name and version")
...
def human(command: str, data: dict[str, object]) -> str:
    ...
    if command in {"check", "diagnostics"}:
        return "\n".join(
            f"ok {entry['name']} {'.'.join(str(part) for part in entry['version'])} {entry['manifest_sha256']}"
            for entry in data["checked"]
        )
    if command == "resolve":
        return f"resolved {len(data['lock']['packages'])} packages; lock {data['lock_sha256']}"
    if command == "build":
        return f"built {data['bytes']} bytes for {data['target']}; package {data['package_sha256']}"
    if command == "inspect":
        return f"{data['name']} {'.'.join(str(part) for part in data['version'])}; manifest {data['manifest_sha256']}"
    if command == "explain":
        capabilities = ", ".join(data["authority"]["capabilities"]) or "none"
        directories = ", ".join(item["name"] for item in data["authority"]["directories"]) or "none"
        return f"{data['name']}: capabilities={capabilities}; directories={directories}; cost={json.dumps(data['cost'], sort_keys=True)}"
    if command == "plan":
        return f"plan {data['lock_sha256']}: {len(data['packages'])} packages for {data['target']}"
